...
exports.StatutPlaying = StatutPlaying



async function StatutPlaying (message){
    if (message.content.startsWith("~Statut")) {
        if (message.content.includes("PLAYING")){
            console.log('hello world')
            const str = (`${message.author.lastMessage}`);
            const Type = (str.substr(8, 7));
            const Custom = (str.substr(16));
